...
Clean complete reference-guided image revision engine.
"""
...
def write_image(path, image, quality=98):
    ...
    output = np.clip(image * 255.0, 0, 255).astype(np.uint8)
    ...
    if ext in (".jpg", ".jpeg"):
        ...
        ok = cv2.imwrite(path, output)

    if not ok:
        raise RuntimeError(f"Cannot write output: {path}")
...
def replace_luminance(image, target_lum):
    ...
    ratio = target_lum / (current_lum + EPS)
...
def apply_highlight(image, normal, over, value):
    value = float(np.clip(value, -1.0, 1.0))
...
def apply_depth(image, normal, value):
    ...
    midtone = midtone_map(normal_lum)
    ...
    protection = (
        smoothstep(normal_lum, 0.03, 0.15)
        *
        (
            1.0
            -
            smoothstep(normal_lum, 0.84, 0.98)
        )
    )
...
def apply_dehaze(image, normal, value):
    """
    Multi-scale reference-guided tonal separation restoration.

    Positive values restore missing local structure relative to
    the Normal exposure. Negative values reduce that separation.
    """
    ...
    cur_fine = current_lum - cur_s
    norm_fine = normal_lum - norm_s

    cur_mid = cur_s - cur_m
    norm_mid = norm_s - norm_m

    cur_broad = cur_m - cur_l
    norm_broad = norm_m - norm_l
    ...
    midtone = midtone_map(current_lum)
...
def apply_contrast(image, value):
    ...
    protection = (
        smoothstep(lum, 0.03, 0.18)
        *
        (
            1.0
            -
            smoothstep(lum, 0.82, 0.98)
        )
    )
...
def apply_color(image, value):
    ...
    # BGR: positive = warmer, negative = cooler.
    result[:, :, 2] *= 1.0 + amount
    result[:, :, 0] *= 1.0 - amount
...
def reference_metrics(image, normal):
    lum = luminance(image)
    normal_lum = luminance(normal)

    shadow_mask = normal_lum < 0.35
...
def refine_with_values(
    ...
    selected = read_image(selected_path)
    ...
    parameters = {
        ...
        "shadow": get_value("shadow"),
        "highlight": get_value("highlight"),
        "depth": get_value("depth"),
        "dehaze": get_value("dehaze"),
        "contrast": get_value("contrast"),
        "saturation": get_value("saturation"),
        "color": get_value("color")
    }

    before = image_metrics(selected)
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="FREE MEF - Correction Engine V3"
    )
...
def print_result(result):
    values = result["feedback_values"]

    before = result["before"]
    after = result["after"]

    before_reference = result["before_reference"]
    after_reference = result["after_reference"]
